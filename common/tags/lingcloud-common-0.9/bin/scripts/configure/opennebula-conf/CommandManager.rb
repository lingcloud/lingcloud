...
private
    ...
    def execute
        if @stdin
            Open3.popen3("ssh -o PasswordAuthentication=no #{@host} #{@command} ; echo ExitCode: $? 1>&2")
        else
            Open3.popen3("ssh -o PasswordAuthentication=no -n #{@host} #{@command} ; echo ExitCode: $? 1>&2")
        end
    end
end
